...
public class WallSceneAnalyzer {
    ...
    private String buildClassifyPrompt(ImageType imageType, int maskCount) {
        ...
        return ("You are looking at an %s photograph. The second image is the SAME photo with\n"
                ...
                + "CRITICAL RULE — EXCLUDE any mask that contains ANY non-paintable material,\n"
                + "even if it also contains some wall. A mask that covers \"wall + window\" or\n"
                + "\"wall + stone column\" must go into EXCLUDE. It is FAR better to lose a\n"
                + "small wall fragment than to include stone, windows, or roof in the paintable area.\n\n"
                + "EXCLUDE — list mask numbers covering ANY of these. This is the critical list:\n"
                + "  * Doors, garage doors, gates, door panels\n"
                + "  * Windows, window glass, balcony glass, glazing, window grilles, shutters\n"
                + "  * Stone cladding, exposed brick, brick parapet, stone wall, stone column,\n"
                + "    stone veneer, natural stone of ANY kind\n"
                + "  * Ceramic tile, marble, granite, wallpaper, wood paneling, vinyl/metal siding,\n"
                + "    fiber cement board, stucco that is part of a decorative non-wall element\n"
                + "  * Roof, roof tile, roof shingles, chimney, roof parapet, roof coping,\n"
                + "    roof overhang (the WALL under the overhang is paintable, the overhang itself is not)\n"
                ...
                + "    address numbers, intercoms\n"
                + "  * Trees, bushes, plants, vines, grass, lawn, hedges, potted plants\n"
                + "  * Cars, motorcycles, bicycles, ladders, scooters\n"
                + "  * Sky, clouds, distant buildings, fences (chain-link, metal, or wood)\n"
                + "  * Ground, dirt, gravel, road, sidewalk, driveway, pavement, curb, manhole\n"
                + "  * People, animals, pets\n"
                + "  * Decorative wrought iron, balcony railings, gate railings (railings only —\n"
                + "    the wall behind them is paintable and goes into main_wall)\n"
                + "  * Balcony floor, balcony slab, cantilevered boxes\n\n"
                ...
                + "RULES OF THUMB\n"
                + "- Be EXTREMELY generous with exclude. If a mask contains even 10%% stone or\n"
                + "  window, EXCLUDE the entire mask.\n"
